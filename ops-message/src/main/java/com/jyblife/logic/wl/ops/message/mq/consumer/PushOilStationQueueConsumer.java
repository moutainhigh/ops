 ...
 */
@Component
@RabbitListener(queues = MqContant.OIL_STATION_QUEUE)
public class PushOilStationQueueConsumer {

	public static Logger logger = LoggerFactory.getLogger(PushOilStationQueueConsumer.class);
	
	@Value("${push.oil.station.api}")
	private String api;
    ...
    public void onMessage(Object msg) {
		...
		logger.info("推送油站消息mq监听参数：" + mqContent);
		
		MqMessage message = JSON.parseObject(mqContent, MqMessage.class);
		String resp = null;
		try {
			Map m = JSON.parseObject(message.getContent(), Map.class);
			logger.info("推送油站数据请求api: " + api + " data: " + message.getContent());
			resp = HttpUtils.post(api, m, "UTF-8");
			logger.info("推送油站数据请求响应: " + resp);
		} catch (Exception e) {
			logger.error("推送油站数据请求异常: " + e.getMessage());
		} finally {
			...
			if(json == null || !"0".equals(json.getString("code"))) {
				...
				if(message.getRetry() > 5) {
					message.setRetryTime(MqContant.OIL_STATION_QUEUE_RETRYTIME);
				}
