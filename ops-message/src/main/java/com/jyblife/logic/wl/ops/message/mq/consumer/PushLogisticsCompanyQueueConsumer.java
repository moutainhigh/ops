...
@Component
@RabbitListener(queues = MqContant.LOGISTICS_COMPANY_QUEUE)
public class PushLogisticsCompanyQueueConsumer {

	public static Logger logger = LoggerFactory.getLogger(PushLogisticsCompanyQueueConsumer.class);
	
	@Value("${push.logistics.company.api}")
	private String api;
    ...
    public void onMessage(Object msg) {
		...
		logger.info("推送物流企业消息mq监听参数：" + mqContent);
		
		MqMessage message = JSON.parseObject(mqContent, MqMessage.class);
		String resp = null;
		try {
			Map m = JSON.parseObject(message.getContent(), Map.class);
			logger.info("推送物流企业数据请求api: " + api + " data: " + message.getContent());
			resp = HttpUtils.post(api, m, "UTF-8");
			logger.info("推送物流企业数据请求响应: " + resp);
		} catch (Exception e) {
			logger.error("推送物流企业数据请求异常: " + e.getMessage());
		} finally {
			...
			if(json == null || !"0".equals(json.getString("code"))) {
				...
				if(message.getRetry() > 5) {
					message.setRetryTime(MqContant.LOGISTICS_COMPANY_RETRYTIME);
				}
